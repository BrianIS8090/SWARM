...
Агенты идентифицируются по имени (`--agent <имя>`) или через файл сессии `.swarm_session_<имя>` + переменные окружения `SWARM_AGENT`/`SWARM_SESSION`. Это позволяет запускать несколько агентов одного типа CLI.
