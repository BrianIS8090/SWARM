...
def create_task(
  description: str, priority: int = 3, target_cli: str | None = None,
  target_name: str | None = None, target_role: str | None = None, depends_on: int | None = None,
) -> Task:
...
def get_all_tasks(
  status: TaskStatus | None = None, assigned_to: int | None = None, priority: int | None = None,
) -> list[Task]:
...
def unlock_file(file_path: str, agent_id: int | None = None, force: bool = False) -> bool:
  ...
  with get_connection() as conn:
    if force:
      ...
      cursor = conn.execute("DELETE FROM file_locks WHERE file_path = ? AND locked_by = ?", (normalized_path, agent_id))
...
def log_event(
  event: EventType, task_id: int | None = None, agent_id: int | None = None,
  message: str | None = None, conn: sqlite3.Connection | None = None,
) -> None:
...
def get_recent_events(
  limit: int = 20, task_id: int | None = None, agent_id: int | None = None,
