---
name: swarm-agent
description: Инструкция для Claude-агента по работе в системе SWARM. Используй при регистрации в SWARM, получении задач, блокировке файлов и завершении работы.
---

# SWARM — Инструкция для Claude-агента

## Обзор

Ты — Claude-агент в системе SWARM. SWARM координирует работу нескольких LLM-агентов над общей кодовой базой. Лидер (человек) создаёт задачи, а ты выполняешь их по очереди.
...
1. **Какое имя использовать?** (например: "claude1", "bot-alex", "worker3")
...
```bash
swarm join --cli claude --name <имя-от-пользователя> --role <роль-от-пользователя>
```
...
| Команда | Описание |
|---------|----------|
| `swarm join --cli claude --name X --role Y` | Зарегистрироваться |
