...
Если после установки команда `swarm` не находится в PowerShell, выполните:

```bash
python -m swarm.cli path
```

Проверить статус без изменений можно так:
...
| `swarm agents --cleanup`                   | Удалить неактивных агентов, освободить задачи и блокировки (`--force` — всех) |
...
| `swarm unlock --file <файл> --agent имя`         | Снять свою блокировку                      |
...
├── lock.py         — lock, unlock
├── common.py       — Общие утилиты команд (_check_agent)
...
* **Одна блокировка на агента** — предотвращение дедлоков
