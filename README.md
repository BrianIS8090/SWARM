...
Откройте отдельный терминал для каждого агента из рекомендации оркестратора ( предложения оркестратора по агентам это лишь совет, пользователь может явно указать сколько и каких агентов CLI он хочет использовать в данном проекте, оркестратор после подстроится под требования пользователя) 
...
codex
#  → swarm join --cli codex --name dev-1 --role developerin

# Терминал 3: Gemini как frontend developer
gemini
# → swarm join --cli gemini --name front-1 --role developerin
```
и т.д.
...
└── start.py        — Команда старта
   ...
   db.py — SQLite WAL, BEGIN IMMEDIATE (атомарный захват задач и блокировок)
        │
   models.py — Agent, Task, FileLock, TaskLogEntry (dataclasses + enums)
```
